...
def get_square_centered(data, count, spec):
    coord1 = []
    coord2 = []
    ...
    coords = [(np.array(coord1), np.array(coord2))]
    return coords
        

def noise_masking(spec, utt_len, mask_label, config):
    s_vs_p = 0.5
    amount = config['amount']
    count = randrange(config['consecutive_start']) + config['consecutive_int'] # 3 + 0~2
    out = np.copy(spec)
    
    new_spec = spec[:utt_len]        
    spec_size = int(new_spec.size(0) * new_spec.size(1))
    
    # Salt mode -> mask with max value (utt-level or dataset-level)
    num_salt = np.ceil(amount * spec_size * s_vs_p)
    coords = [np.random.randint(0, i - 1, int(num_salt)) for i in new_spec.shape]
    output_salt = get_square_centered(coords, count, new_spec)
    
    if config['salt_level'] == 'utt':
        max_val = torch.max(spec)
    else:
        max_val = config['salt_value']
    out[output_salt[0]] = max_val
    mask_label[output_salt[0]] = 1 # mask label 
    
    # Pepper mode -> mask with zero value
    num_pepper = np.ceil(amount * spec_size * (1. - s_vs_p))
    coords = [np.random.randint(0, i - 1, int(num_pepper)) for i in new_spec.shape]
    output_pepper = get_square_centered(coords, count, new_spec)
    out[output_pepper[0]] = config['pepper_value']
    mask_label[output_pepper[0]] = 1 # mask label
    
    return torch.Tensor(out), torch.Tensor(mask_label)


def generate_masked_acoustic_model_data_sp(spec, config):
    ...
    with torch.no_grad():
        ...
        for idx in range(batch_size):
            ...
            # s&p masking
            sp_noise_output, sp_noise_mask_label = noise_masking(spec_masked[idx], spec_len[idx], mask_label[idx].data.cpu().numpy(), config)
        
            spec_masked[idx] = sp_noise_output
            mask_label[idx] = sp_noise_mask_label
            
            # allow conventional masking
            if config['conventional_mask']:
                # time masking
